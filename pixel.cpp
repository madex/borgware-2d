...
extern "C" {
  ...
  void setpixel(pixel p, unsigned char value ){
    if (p.x >= 16) {
      Serial.println("x zu hoch");
    }
    if (p.y >= 16) {
      Serial.println("y zu hoch");
    }
  	p.x %= NUM_COLS;
  	if (value > NUMPLANE) {
  		value = NUMPLANE;
  ...
  void shift_pixmap_l() {
  	unsigned char plane, row, by;
  	for (plane = 0; plane<NUMPLANE; plane++) {
  		for (row = 0; row<NUM_ROWS; row++) {
  			for (by = 0; by < (LINEBYTES-1); by++) {
  				pixmap[plane][row][by] >>= 1;
  				pixmap[plane][row][by] |= (pixmap[plane][row][by+1] & 0x01) * 0x80;;
  			}
